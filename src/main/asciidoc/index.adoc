= Order-service Getting Started Guide
:doctype: book
:icons: font
...
Order-service is a RESTful microservice for handling client's orders.
...
[[resources-order]]
== Order
The Order resource is used to create, modify and list order.

[[resource-order-list]]
=== Listing order
A `GET` request lists all of the service's order.
...
=== Creating order
A `Post` request for creating order.
